...
func OaiStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
    ...
    if responseText != "" {
        ...
        if len(responseText) > maxLength {
            responseText = responseText[:maxLength] + "..."
        }
        c.Set("ai_response_content", responseText)
    }
...
func OpenaiHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
    ...
    // Extract AI response content for logging
    if len(simpleResponse.Choices) > 0 {
        ...
        for _, choice := range simpleResponse.Choices {
            ...
            if choice.Message.ReasoningContent != "" {
                ...
                aiResponse.WriteString(choice.Message.ReasoningContent)
            }
            if choice.Message.Reasoning != "" {
                if aiResponse.Len() > 0 {
                    aiResponse.WriteString("\n")
                }
                aiResponse.WriteString(choice.Message.Reasoning)
            }
        }
        if aiResponse.Len() > 0 {
            responseText := aiResponse.String()
            const maxLength = 2000
            if len(responseText) > maxLength {
                responseText = responseText[:maxLength] + "..."
            }
