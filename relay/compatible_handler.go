...
func postConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent string) {
    ...
    // Extract user question and AI response for consumption type logs
    if textReq, ok := relayInfo.Request.(*dto.GeneralOpenAIRequest); ok && len(textReq.Messages) > 0 {
        var userQuestion string
        
        for i := len(textReq.Messages) - 1; i >= 0; i-- {
            msg := textReq.Messages[i]
            if msg.Role == "user" {
                userQuestion = msg.StringContent()
                break
            }
        }
        ...
        if userQuestion != "" {
            ...
            if len(userQuestion) > maxLength {
                userQuestion = userQuestion[:maxLength] + "..."
            }
            other["user_question"] = userQuestion
        }

        aiResponseContent := ctx.GetString("ai_response_content")
        if aiResponseContent != "" {
            other["ai_response"] = aiResponseContent
        }
    }
